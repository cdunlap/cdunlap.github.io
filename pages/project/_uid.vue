...
main#project(v-if="Object.keys(project).length > 0")
  b-jumbotron(
    :fluid="true",
    :header="$prismic.asText(project.data.name)",
    :lead="$prismic.asText(project.data.description)"
  )
    .tags
    ...
    b-row
      ...
      b-col(md="6")
        .embed-responsive.embed-responsive-16by9(
          v-if="project.data.demo_video && project.data.demo_video.url"
        )
          video(:src="project.data.demo_video.url", autoplay, muted, controls)
        prismic-image.img-fluid(
          v-if="project.data.hero_image && project.data.hero_image.url",
  ...
  b-container(v-for="(slice, idx) in project.data.body", :key="`slice-${idx}`")
    template(v-if="slice.slice_type === 'project_photos'")
      h2 Photos
      v-gallery(
        :images="slice.items.map((i) => i.project_photo.url)",
        :index="photoIndex",
        @close="photoIndex = null"
      )
      .gallery-photos.row
        figure.col-md-4.col-sm-2(
